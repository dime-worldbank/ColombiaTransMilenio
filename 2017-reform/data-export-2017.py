...
# MAGIC ## 6. Stata import do-file → `import_2017.do`
# MAGIC
# MAGIC Generated from the same code maps as the CSVs, so the value labels can never drift from the exported codes. The do-file uses the project's directory globals (`directories.do` in the Colombia-BRT-IE repo): it reads the downloaded CSVs from `$Int_2017`, converts the month strings to Stata monthly dates, applies the value labels and saves compressed `.dta` files back to `$Int_2017`. Final analysis datasets are saved to `$Final_2017` by the later construction/anonymization steps, not here.
...
do_file = f"""* ============================================================
* DO-FILE: import_2017.do — GENERATED FILE, do not edit by hand.
* ============================================================
...
*
* INPUT : $Int_2017\\cards_2017.csv, $Int_2017\\panel_2017.csv
*         (place the downloaded CSVs in $Int_2017)
* OUTPUT: $Int_2017\\cards_2017.dta, $Int_2017\\panel_2017.dta
*         (final analysis datasets are saved to $Final_2017 by the
*          construction and anonymization steps, not here)
* ============================================================
...
* Directories
* ============================================================
	global Repo  "C:\\Users\\\\`c(username)'\\GitHub\\Colombia-BRT-IE\\Subsidy Paper"

	do "$Repo\\Globals\\directories.do"

* ============================================================
* Cards (one row per card, ALL cards in the window)
* ============================================================
	import delimited using "$Int_2017\\PII\\cards_2017.csv", clear stringcols(2) case(preserve)

	{label_define("treatment", TREATMENT_CODES)}
	label values treatment treatment

	compress
	save "$Int_2017\\PII\\cards_2017.dta", replace

* ============================================================
* Panel (balanced card x month, analysis sample only)
* ============================================================
	import delimited using "$Int_2017\\panel_2017.csv", clear case(preserve)
	...
	compress
	save "$Int_2017\\panel_2017.dta", replace

* Panel setup after merging card-level variables (the panel's cards are
* exactly the cards with in_sample==1):
*   use "$Int_2017\\panel_2017.dta", clear
*   merge m:1 card_id using "$Int_2017\\cards_2017.dta", keep(match) nogen
*   xtset card_id mdate
"""
