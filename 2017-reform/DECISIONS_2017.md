...
| T6. Fare table | group × fare period × fare type | Modal fare, frequency and share for `zonal`, `troncal` and the four transfer types (E22). Long format, exported as is | computed over the sample cards (T5) |
...
| S4 | Stata analysis (`$Analysis_2017`, adapting `sample-code/analysis/*.do`) | results | Drafted 2026-09-02: event studies, DiD ATEs and arc elasticities; Sisbén heterogeneity skips until S2 runs. Since 2026-09-04 the months dropped from the regressions (event studies, DiD, elasticities, Sisbén heterogeneity) are a single switch in MAIN (`$excl_months`, next to `$excl_infrequent`): default Mar 2017 only (base = Feb 2017); `"-1 -2"` reproduces the earlier Feb–Mar exclusion, `""` keeps all months; outputs carry a suffix (`exMar`, `exFebMar`, `all`) so runs with different exclusions coexist. Trend-adjusted event studies (`xtevent`, linear differential pre-trend, added 2026-09-03 from the sample-code detrend files) parked 2026-09-04: removed from MAIN's call list, file kept with its own hardcoded variants; not pursued for now. Pending from the sample code: HonestDiD smoothness bounds. Rewritten 2026-09-04: `Analysis_Reform_2017_elasticities.do` runs on the fares and prices files (no hand-typed fare globals) — price table by group (apoyo_kept in total and by Sisbén range, apoyo_lost, never) × window (6m, 3m) × concept (nominal zonal/troncal/transfers, nominal weighted by the basket, effective = paid per trip vs same basket at post fares, observed post as diagnostic), each change also net of the never-treated change (price DiD); arc elasticities per group × window × concept with the differential price change as baseline and the raw one alongside; by Sisbén range the betas come from the Sisbén DiD, now also saved as `.ster`. Needs Stata 16+ (frames). Details: `local-code/Analysis/Reform_2017/README.md` |
...
| E22 | Fare table with transfers (T6) | Trips: top-2 modal values by `card_group` × fare period over each card's first 30 trips of the month (as E1) → `zonal` (lower), `troncal` (higher). Transfers: modal value by `card_group` × fare period × transfer type → `tr_zz`, `tr_zt`, `tr_tz`, `tr_tt`. Long format with frequency and share. A transfer type with no transfers in a group-period gets the base transfer fare (`tr_zz`) and is reported. The full value distribution of transfers by type is displayed as the check of the pricing rule: 300 for every transfer before the reform; after it, 200 for zonal → troncal and 0 otherwise (to be read off the run, not assumed) | ✅ written |
...
* E24. What Stata does with T6 and T8 (S4, written 2026-09-04): the price table by group and window — nominal (zonal, troncal, transfers, and the average weighted by the group's basket shares) and effective (`p_pre`, `p_post_cf`, `p_post_obs`) — for `apoyo_kept` (in total and by `sisbenIII_range` once S2 runs), `apoyo_lost` and `never`; the **price DiD** = change of the treated group minus change of `never` (never also faces a fare change: the full fares rose in April 2017), in % and in levels; and the arc elasticities with both the nominal and the effective price change, the differential one as baseline. Windows are exported both ways so the choice (6m vs 3m, in line with or apart from `$excl_months`) is made in Stata.
