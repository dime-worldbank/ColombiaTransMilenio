...
| N2 | window + Cleaning-2 notebook (new) | T2 | To create: read T1, filter to the window by filenames (C1), compute tags C2–C7, add Level-1 constructed columns (E2, E3, E5, E6) incl. switch tags (§D), save T2. E4 is NOT built here: it needs the fare table (E1) and `ever_*` (E12), both built in N3 (decided 2026-08-31) |
| N3 | construction & aggregation notebook (new; may be split in two) | T3, T4, T5 | To create: `ever_*` (E12); fare table E1 built inline right before its first use, E4 (`apoyo_trip`/`mayor_trip`, moved here from N2; with the pre-tabulation of users at subsidized fares per month); remaining card-level vars E13–E16; monthly outcomes E10–E11; balanced panel E18–E19; save T3–T5 |
