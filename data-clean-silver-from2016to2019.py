...
# MAGIC 3. **Ingestion diagnostics** — control table vs bronze, encoding, duplicate ingestion
# MAGIC 4. **Dates** — parse `fecha_transaccion` and `clearing_date`
# MAGIC 5. **Data coverage** — month×day heatmap and daily plots (raw data)
# MAGIC 6. **Window files** — completeness of Oct 2016 – Sep 2017 and duplicate extra files
...
# The 12 monthly files that cover the analysis window
MONTHLY_FILES = [
...
# Files excluded from silver: 10 files dated Sep 30 2017 whose content duplicates
# the monthly files (their rows match on card + exact timestamp). The pattern
# catches all of them and cannot match any monthly file:
...
# DBTITLE 1,Section 2: Bronze table
# MAGIC %md
# MAGIC ---
# MAGIC ## 2. Bronze table

# COMMAND ----------

# DBTITLE 1,Inspect columns
...
# DBTITLE 1,Control table vs bronze: all non-broken files ingested?
...
df_broken = spark.table(CONTROL_TABLE).filter(F.col("classification_status") == "broken")
...
# DBTITLE 1,Encoding issues: letters in fecha_transaccion
# Any letter other than 'UTC' in the date string indicates encoding corruption.
df_bronze = spark.table(BRONZE_TABLE)

df_weird = df_bronze.filter(
    F.regexp_replace(F.col("fecha_transaccion"), "(?i)UTC", "").rlike("[A-Za-z]")
)
...
# DBTITLE 1,Duplicate ingestion check (optional, expensive)
# Compares total vs distinct content rows per file. Set to True to run.
RUN_DUPLICATE_CHECK = False

if RUN_DUPLICATE_CHECK:
    content_cols = [c for c in df_bronze.columns if c not in META_COLS]
...
# DBTITLE 1,Raw file row counts vs bronze (optional, for specific files)
# List files here to compare their raw line count against their bronze row count.
files_to_verify = []
...
# DBTITLE 1,Section 4: Dates
# MAGIC %md
# MAGIC ---
# MAGIC ## 4. Dates
# MAGIC
# MAGIC Parses `fecha_transaccion` (string → date + timestamp) and `clearing_date` (string → date; it has no time in the source). Original strings are preserved as `*_string`.

# COMMAND ----------

# DBTITLE 1,Classify formats, parse, and check nulls
date_vars = ["fecha_transaccion", "clearing_date"]

# Detect which of the known date formats each row uses
...
# Spark parsing patterns for each detected format
_time_formats = {
...
def _build_ts_expr(string_col, type_col):
    # Timestamp: only formats that carry a time
...
# Format distribution and unknown formats per variable
...
# Parse: keep original strings, overwrite the columns with proper date types
...
# Null counts after parsing (only 'unknown' format rows should be null)
...
for var in date_vars:
    ...
    print(f"Null {var} (date)    : {null_dt:,}  ({100*null_dt/total:.1f}%)")
    if var == "fecha_transaccion":
        null_ts = df_with_parsed_dates.filter(F.col("fecha_transaccion_timestamp").isNull()).count()
        print(f"Null {var}_timestamp : {null_ts:,}  ({100*null_ts/total:.1f}%)")

# COMMAND ----------

# DBTITLE 1,Section 5: Data coverage
# MAGIC %md
# MAGIC ---
# MAGIC ## 5. Data coverage (raw data)

# COMMAND ----------

# DBTITLE 1,Month × day heatmap of daily rows
# Gray = day with data, red = valid day with no data, white = invalid date.
daily_pd = (
...
plt.suptitle("Daily rows heatmap (month × day, clearing_date) — raw data",
...
# DBTITLE 1,Daily transactions line plots
daily_pd["clearing_date"] = pd.to_datetime(daily_pd[["year", "month", "day"]])
...
for ax, df_plot, title_var in [(ax1, full_df, "clearing_date"), (ax2, full_tx_df, "fecha_transaccion")]:
    ...
    ax.set_title(f"Daily observations by {title_var} — raw data", fontsize=14, fontweight="bold")
...
# DBTITLE 1,Section 6: Window files
# MAGIC %md
# MAGIC ---
# MAGIC ## 6. Window files
# MAGIC
# MAGIC Checks that Oct 2016 – Sep 2017 is fully covered by the 12 monthly files, and that the extra files contributing rows to that window are duplicates of the monthly files (they match on card + exact timestamp), so they can be excluded.

# COMMAND ----------

# DBTITLE 1,Files contributing rows to the window + missing days
...
# Days in the window with no rows at all
...
# DBTITLE 1,Extra files vs monthly files: duplicate check (card + timestamp)
# For each non-monthly file with rows in the window: what share of its rows
# already exists in the monthly files, matching on card + exact timestamp?
# ≈100% → duplicate file, safe to exclude. ≈0% → new data, do NOT exclude.
...
# Overlap on card + exact timestamp
...
match_card_ts = (
    key_extra.join(key_monthly, ["card_k", "ts_k"], "left_semi")
    .groupBy("_source_file").agg(F.count("*").alias("rows_matched"))
)

display(
    totals
    .join(match_card_ts, "_source_file", "left")
    .fillna(0, subset=["rows_matched"])
    .withColumn("pct_matched", F.round(100 * F.col("rows_matched") / F.col("rows_in_window"), 2))
    .orderBy("_source_file")
)

# COMMAND ----------

# DBTITLE 1,Section 7: Numeric variables
# MAGIC %md
# MAGIC ---
# MAGIC ## 7. Numeric variables

# COMMAND ----------

# DBTITLE 1,Parseability check + value distribution
numvars = ['cardnumber', 'balance_before', 'value', 'balance_after']

# Count rows whose raw value is non-empty but cannot be parsed to a number
...
# DBTITLE 1,Section 8: Categorical variables
# MAGIC %md
# MAGIC ---
# MAGIC ## 8. Categorical variables
# MAGIC
# MAGIC Each raw value is mapped to a canonical string via explicit dictionaries. Unexpected values fall to NULL and are counted in the coverage checks.

# COMMAND ----------

# DBTITLE 1,card_type raw values
spark.table(BRONZE_TABLE).select('card_type').groupBy('card_type').count().display()

# COMMAND ----------

# DBTITLE 1,Maps: emisor, operator, card_type
# Issuer
_issuer_map = {
...
# DBTITLE 1,Map: account_name → card_profile
# Keys are the exact trimmed raw strings found in bronze. The code in
# parenthesis is ambiguous for this variable ((001), (003), (006) and (029)
# each cover two different profiles), so the map keys on the FULL string.
# The map only fixes encoding corruption and truncated values — no detail is
# lost; grouping profiles for analysis happens downstream.
_profile_canonical = [
    "(001) Adulto",
    "(001) Anonymous",
    "(002) Adulto Mayor",
...
_profile_variants = {
    ...
    "(101)": "(101) Adulto PV",
    # "(000)" and "1" match no profile: left unmapped → NULL
}
...
# DBTITLE 1,Map coverage checks
# For each mapped variable: full raw × mapped table (NULL results first), and
# count of unmapped non-null raw values (candidates to add to the maps).
cat_maps = {
...
# DBTITLE 1,Section 9: Missing values
# MAGIC %md
# MAGIC ---
# MAGIC ## 9. Missing values

# COMMAND ----------

# DBTITLE 1,Share of empty content columns per row
# Rows with 100% missing content are dropped when building silver.
# Rows with many (but not all) missing columns are shown here to inspect.
_content_cols = [c for c in spark.table(BRONZE_TABLE).columns if c not in META_COLS]
...
# Distribution of the per-row missing share
display(
    df_with_parsed_dates
...
# Sample of high-missing (but not fully empty) rows
display(
    df_with_parsed_dates
...
# DBTITLE 1,Section 10: Build silver
# MAGIC %md
# MAGIC ---
...
df_silver = (
    ...
    # Card profile map (raw account_name kept alongside)
    .withColumn("card_profile", card_profile_expr)
    # Numeric casts (overwrite in place; raw strings remain in bronze)
...
# DBTITLE 1,Check: NULLs introduced by casts and maps
...
print("\n── Unmapped account_name values (expected: only '(000)' and '1') ──")
display(
    df_silver
...
df_dedup_flagged = (
    ...
    # Station and file of the kept (first) row, for the diagnostics
    .withColumn("_kept_station", F.first("station_id").over(_w_order))
    .withColumn("_kept_file", F.first("_source_file").over(_w_order))
)

# Keep only the affected groups in memory for the diagnostics
df_dups = (
    df_dedup_flagged
...
#   same source file (device double-write) or different file (same trip in two files)
_diag = (
    df_dups
    .filter(F.col("_rn") > 1)
    .agg(
...
print(f"  station DIFFERS from the kept row     : {_diag['dropped_total']-_diag['station_match']:,} ({100*(_diag['dropped_total']-_diag['station_match'])/_tot:.1f}%)")
print(f"  SAME _source_file as the kept row     : {_diag['same_file']:,} ({100*_diag['same_file']/_tot:.1f}%)")
print(f"  DIFFERENT _source_file from kept row  : {_diag['dropped_total']-_diag['same_file']:,} ({100*(_diag['dropped_total']-_diag['same_file'])/_tot:.1f}%)")
...
# DBTITLE 1,Section 12: Status figures
# MAGIC %md
# MAGIC ---
# MAGIC ## 12. Status figures
# MAGIC
# MAGIC 1. Daily transactions, bronze vs silver
# MAGIC 2. Month×day coverage heatmap of silver, analysis window marked
# MAGIC 3. Cleaning waterfall: rows at each cleaning step
# MAGIC 4. Dropped duplicates per month
...
for ax, (var, d_bronze, d_t1) in zip(axes, _daily_pairs):
    ...
    ax.plot(d_t1["date"], d_t1["count"], color="steelblue", linewidth=0.8, label="Silver (clean)")
    ...
    ax.set_title(f"Daily transactions by {var} — bronze vs silver", fontsize=13, fontweight="bold")
...
# DBTITLE 1,Figure 2: silver coverage heatmap, analysis window marked
daily_t1_heat = (
    df_t1_tbl
    .filter(F.col("clearing_date").isNotNull())
    .groupBy("clearing_date")
...
for ax, year in zip(axes, years_t1):
    ...
    # Blue frame around the analysis-window months of this year
    ...
    red_p   = mpatches.Patch(color="red", label="No data (valid day)")
    white_p = mpatches.Patch(facecolor="white", edgecolor="lightgray", label="Invalid date")
    win_p   = mpatches.Patch(facecolor="none", edgecolor="dodgerblue", linewidth=2, label="Analysis window")
    ax.legend(handles=[red_p, white_p, win_p], loc="lower right", fontsize=8, framealpha=0.85)

plt.suptitle("Silver coverage heatmap (month × day, clearing_date), analysis window marked",
             fontsize=14, fontweight="bold", y=1.01)
plt.tight_layout()
plt.show()
...
# DBTITLE 1,Figure 3: cleaning waterfall
# Rows at each cleaning step, using the counts computed above.
...
if _expected_t1 == n_t1:
    print(f"✅ Consistent: bronze − exclusions = silver rows ({n_t1:,})")
else:
    print(f"⚠️  Mismatch: expected {_expected_t1:,} but silver has {n_t1:,} (diff {n_t1 - _expected_t1:+,}) — investigate")
...
ax.set_title("Cleaning waterfall: rows at each step (count and % dropped vs previous)",
...
# DBTITLE 1,Figure 4: dropped duplicates per month
